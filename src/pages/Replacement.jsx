...
import { sendOtp, validateOtp, replaceTag, getAgentTags } from '../services/api'
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <h1 className="text-2xl font-bold text-white">Tag Replacement</h1>
        ...
        {STEPS.map((s, i) => (
          <div key={s.id} className="flex items-center flex-shrink-0">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all
              ${step === s.id ? 'bg-brand-600 text-white' : step > s.id ? 'bg-brand-900/50 text-brand-400' : 'bg-gray-800 text-gray-500'}`}>
              <span className="w-4 h-4 rounded-full bg-current/20 flex items-center justify-center text-[10px]">{s.id}</span>
              {s.title}
            </div>
            {i < STEPS.length - 1 && <ChevronRight size={14} className="text-gray-700 mx-1 flex-shrink-0" />}
          </div>
        ))}
      </div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="r1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <h2 className="text-lg font-semibold text-white mb-4">Enter Vehicle Details</h2>
          ...
          {step === 3 && (
            <motion.div key="r3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <h2 className="text-lg font-semibold text-white mb-4">Select Replacement Tag</h2>
              {vehicleDetails && (
                <div className="bg-gray-800/60 rounded-xl p-4 mb-5 text-sm">
                  <p className="text-brand-400 font-semibold">{vehicleDetails.vehicleNo}</p>
                  <p className="text-gray-400 mt-1">{vehicleDetails.vehicleManuf} {vehicleDetails.model}</p>
                </div>
              )}
              <div className="mb-4">
                <label className="text-sm text-gray-400 mb-2 block">Reason for Replacement *</label>
              ...
              <label className="text-sm text-gray-400 mb-2 block">Select New Tag *</label>
              {agentTags.length === 0
                ? <div className="text-center py-8 text-gray-500 text-sm bg-gray-800/40 rounded-xl">No tags available in inventory</div>
          ...
          {step === 4 && (
            <motion.div key="r4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              <div className="text-center py-8">
                ...
                <h2 className="text-2xl font-bold text-white mb-2">Tag Replaced!</h2>
                ...
                {result && (
                  <div className="bg-gray-800/60 rounded-2xl p-5 text-left space-y-3 mb-6">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white font-mono">{result.vrn}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">New Serial No</span>
                      <span className="text-white font-mono">{result.serialNo}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">NPCI Status</span>
                      <span className="text-green-400">{result.npciStatus}</span>
                    </div>
                  </div>
                )}
